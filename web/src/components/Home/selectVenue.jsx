...
class SelectVenue extends React.Component {
  // Method to restore the last used values from the local storager
  // @param: takes in the variable name used to save last used value in the local storager
  ...
  getLocations = () => {
    let filteredVenues = this.props.venues.filter(elt => {
      return (
        elt.category === this.props.category &&
        elt.venue_name === this.props.venue
      );
    });
    ...
    return (
      <Grid container className={styles.venueLocation}>
          ...
          <Select
            ...
            onChange={event => {
              ...
              localStorage.setItem(
                "categoryValueInLocalStorage",
                event.target.value
              );

              console.log(event.target.value);
              console.log(localStorage.getItem("categoryValueInLocalStorage"));
            }}
          >
        ...
        <Grid item xs={12} md={6}>
          <Select
            style={{ width: "10rem", textAlign: "left" }}
            disabled={this.props.category === "_noCategory"}
          ...
          >
            ...
            {venues.map(venue => (
              <MenuItem value={venue} key={venue}>
                {" "}
                {venue}{" "}
              </MenuItem>
            ))}
        ...
        <Grid item xs={12} md={12}>
          <Select
            ...
            disabled={
              this.props.category === "_noCategory" ||
              this.props.venue === "_noVenue"
            }
          ...
          >
            ...
            {locations.map(venue => (
              <MenuItem value={venue.venue_uid} key={venue.venue_uid}>
                {" "}
...
export default connect(
  mapStateToProps,
  { selectCategory, selectVenue, selectLocation }
)(SelectVenue);
